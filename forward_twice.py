...
def writeDatabase(dic):
    ...
    for k in range(len(dic)):
        ...
        sql = 'INSERT INTO ' + key + '(博主昵称, 博主主页, 内容, 微博认证 , 发布时间, 转发, 赞) VALUES ('
...
def getContent():
    ...
    for i in range(len(nodes)):
        ...
        try:
            BZNC = nodes[i].find_element_by_xpath(".//div[@class='WB_text']/a[@node-type='name']").text
        except:
            BZNC = ''
        # print u'博主昵称:', BZNC
        dic[i].append(BZNC)
        ...
        try:
            WBNR = nodes[i].find_element_by_xpath(".//div[@class='WB_text']/span[@node-type='text']").text
        except Exception as e:
            WBNR = ''
        # print '微博内容:', WBNR
        dic[i].append(WBNR)
        ...
        try:
            WBRZ = nodes[i].find_element_by_xpath(".//a[@suda-data='key=pc_apply_entry&value=feed_icon']/i[@class = 'W_icon icon_approve']").get_attribute('title')#若没有认证则不存在节点
            WBRZ += '(已认证)'
            # print WBRZ
        except:
            WBRZ = ''
        ...
        if WBRZ == '':
            try:
                WBRZ = nodes[i].find_element_by_xpath(".//a[@suda-data='key=pc_apply_entry&value=feed_icon']/i[@class = 'W_icon icon_approve_co']").get_attribute('title')#若没有认证则不存在节点
                WBRZ += '(企业号)'
                # print WBRZ
            except:
                WBRZ = ''
        ...
        if WBRZ == '':
            try:
                WBRZ = nodes[i].find_element_by_xpath(".//a[@suda-data='key=pc_apply_entry&value=feed_icon']/i[@class = 'W_icon icon_approve_gold']").get_attribute('title')#若没有认证则不存在节点
                WBRZ += '(vip)'
                # print WBRZ
            except:
                WBRZ = ''
        ...
        try:
            FBSJ = nodes[i].find_element_by_xpath(".//div[@class='WB_from S_txt2']/a[@class='S_txt1']").get_attribute('title')
        except:
            FBSJ = ''
        # print u'发布时间:', FBSJ
        dic[i].append(FBSJ)
        ...
        try:
            ZF_TEXT = nodes[i].find_element_by_xpath(".//ul[@class='clearfix']/li[2]/span[@class='line S_line1']/a[@class='S_txt1']").text
            if ZF_TEXT == '':
                ZF = 0
            else:
                ZF = int(ZF_TEXT[3:])
        ...
        try:
            ZAN_TEXT = nodes[i].find_element_by_xpath(".//span[@node-type='like_status']/em[2]").text #可为空
            if ZAN_TEXT == '':
                ZAN = 0
            else:
                ZAN = int(ZAN_TEXT)
